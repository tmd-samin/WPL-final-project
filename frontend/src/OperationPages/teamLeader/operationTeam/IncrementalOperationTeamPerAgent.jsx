  ...
  return (
    <>
      <div className="titleOperationAndAgents">
        <h3>{`Choosing the cumulative display by month: ${moment(
          selectedMonth,
          "MM/YYYY"
        ).format("MM/YYYY")}`}</h3>
        <div>
          <FormControl
            ...
            <Select
              ...
              <MenuItem value={selectedAgent}>
                <em>Their agents are cumulative</em>
              </MenuItem>
              {agents.map((agent) => (
      ...
      <>
        {!filteredOperations.length ? (
          <div className="titleOperationAndAgents">
            <h3>{`There is no cumulative / monthly operation for the agent${selectedAgent}`}</h3>
          </div>
        ) : (
          <>
            {
